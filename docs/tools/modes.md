...
CLI `surfaces` / `--json`, TUI surface tabs (`r`), and Desktop Connection **Probe surfaces** share one probe report (SSH, CLI, API, MCP PC + box). Footer / rows use `local=` and `box=probing` until the probe finishes.
