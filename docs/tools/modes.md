...
```bash
# Make (defaults: REMOTE=horto, RELEASE_TAG=dev-preview, plan only; APPLY=1 for --apply)
...
# Explicit flags (plan = omit --apply)
horto-os-ui --remote horto setup run --full
horto-os-ui --remote horto-box setup status --full
horto-os-ui --remote horto --apply setup run --full
horto-os-ui --remote horto-box --bin-dir target/debug doctor
horto-os-ui --remote horto-box --install-ssh-key doctor

# Same via env (--remote ← HORTO_REMOTE_HOST)
export HORTO_REMOTE_HOST=horto
export HORTO_RELEASE_TAG=dev-preview
horto-os-ui --install-ssh-key doctor
horto-os-ui setup run --full
horto-os-ui --apply setup run --full
```
