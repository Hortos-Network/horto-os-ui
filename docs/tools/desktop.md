...
Desktop is **always remote**: Connection talks HTTP to the status API, and runs
first-time OpenSSH setup through the same shared runner as CLI/TUI (system askpass;
key install off unless checked). Apply is off by default (preview only); check Apply
and confirm to install CLI + TUI + status-api + MCP on the box. See [modes.md](modes.md).
...
After a successful remote apply, Desktop offers to save the status-api bearer
