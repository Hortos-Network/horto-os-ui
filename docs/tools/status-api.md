...
No Trunk proxy is required: the browser talks to the status API on `:8787`. CORS allows localhost / Tauri UI origins. A `Failed to fetch` to the box hostname with a loopback-only bind is a listen-address problem, not CORS.
